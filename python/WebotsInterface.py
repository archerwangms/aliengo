...
class webot_interface:
    def __init__(self) -> None:
        ...
        self.supervisor_name = "Aliengo"
        self.aliengo_joint_name = [['FL_hip_joint','FL_thigh_joint','FL_calf_joint'],
                              ['FR_hip_joint','FR_thigh_joint','FR_calf_joint'],
                              ['RL_hip_joint','RL_thigh_joint','RL_calf_joint'],
                              ['RR_hip_joint','RR_thigh_joint','RR_calf_joint']]
        self.aliengo_joint_sensor_name =  [['FL_hip_joint_sensor','FL_thigh_joint_sensor','FL_calf_joint_sensor'],
        ...
        self.p_gains = torch.tensor(p,dtype=torch.float)
        ...
        if(self.a1_supervisor_node.getDef() != 'Aliengo'):
            print("webots don't have robot named Aliengo \n")
            print(f"node name : ",self.a1_supervisor_node.getDef())
# motor and sensor node        
        self.np_joint_name = np.array(self.aliengo_joint_name)        # print(np_joint_name)        
        for leg_num in range(self.np_joint_name.shape[0]):
            for joint_num in range(self.np_joint_name.shape[1]):
                self.joint_node.append(self.a1_supervisor_class.getDevice(self.aliengo_joint_name[leg_num][joint_num]))
                self.joint_node[leg_num*3+joint_num].enableTorqueFeedback(self.sim_time_step)
                self.position_sensor_node.append(self.a1_supervisor_class.getDevice(self.aliengo_joint_sensor_name[leg_num][joint_num]))
    ...
    def get_keyboard_input(self):
        ...
        self.commands = torch.clip(self.commands,-self.max_velo_x,self.max_velo_x)
